...
但实际上计算洛朗级数时仍然使用泰勒级数的结论 [常用泰勒级数](https://alkaid.github.io/posts/complex_func/taylor_series/)
...
- 内半径$R_1$：从奇点到内侧边界的距离，用负幂项求得
- 外半径$R_2$：从奇点到外侧边界的距离，用正幂项求得
...
- $\frac{1}{z-i}$的奇点$z=i$在圆环内侧，所以$\frac{1}{z-i}$应展开为负幂项
- $\frac{1}{z-2}$的奇点$z=2$在圆环外侧，所以$\frac{1}{z-2}$应展开为正幂项

#### 第三步 展开每一项

之所以需要变为$\frac{1}{z}$，是为了将展开后的收敛域"翻转"至边界的外侧

<!-- >> 我觉得就是靠倒数把$<$某某的收敛域翻过来变成$>$某某，这样就能让z在收敛域内。作为面向初学者的教材还是先给个哪怕不严谨的直观理解会更好吧？-->
