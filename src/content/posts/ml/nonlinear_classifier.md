...
选让分完后数据**最不混乱**的那个特征来分类

**熵** 衡量混乱程度（详见[特征提取](/posts/feature_extraction/)），数据集 $D$ 的熵：

$$
H(D) = -\sum_{k} p_k \log_2 p_k
$$

$p_k$ 是第 $k$ 类占的比例。熵越大越乱，全是同一类时熵=0。

按特征 $A$ 的每个取值 $v$ 把数据分成若干子集 $D^v$，各子集的熵加权平均就是**条件熵**：
...
- $|D|$：总样本数
- $|D^v|$：特征 $A=v$ 的样本子集的样本数
- $\dfrac{|D^v|}{|D|}$：$A=v$ 的样本占全部的比重
- $H(D^v)$：样本子集自己的熵
...
ID3的缺点：过于偏好取值多的特征（比如"编号/ID"这种每个样本值都不同，增益直接拉满但不泛化）。C4.5引入**固有值**来惩罚取值多的特征，增益除以它进行校正：
