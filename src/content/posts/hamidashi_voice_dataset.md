...
- ~~[Node.js](https://nodejs.org/)，写脚本用（~~不太习惯用Python之类的写💦~~）~~ **既然这次用Lua了就直接全用Lua写算了**
...
```
一开始看到这神奇的语法还以为是什么自创的标记语言，然后跟AI对线一小时整了个正则表达式

后来注意到从1开始，于是推测大概率跟lua有关系，问了下AI才知道原来这整个文件就是个lua脚本（~~电脑里有一款索引从1开始的语言~~）
