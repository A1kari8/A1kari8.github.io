...
:::note
真实状态$x$是我们不知道的，而卡尔曼滤波的目的就是通过**测量值**和系统模型估计$\hat x$来尽量接近真实状态$x$
:::
...
举个例子，假设我们在$k$时刻对直线运动的物体施加一个力$force$作为控制输入$u_k$
...
$$
\Delta a = \frac{force}{m}
$$

因为卡尔曼滤波是个**离散系统**，所以我们不关心加速度的连续变化，而只关心在每个时间步长$\Delta t$内加速度的变化量$\Delta a$
...
\Delta t \frac{1}{m}\\
\frac{1}{m}
\end{bmatrix} force = \begin{bmatrix}
\Delta positon \\
\Delta velocity \\
\Delta acceleration
\end{bmatrix}
